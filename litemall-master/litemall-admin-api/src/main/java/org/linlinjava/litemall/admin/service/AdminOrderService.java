...
public class AdminOrderService {
    ...
    public Object refund(String body) {
        ...
        if (!order.getOrderStatus().equals(OrderUtil.STATUS_REFUND) && !order.getOrderStatus().equals(OrderUtil.STATUS_PAY) ) {
            return ResponseUtil.fail(ORDER_CONFIRM_NOT_ALLOWED, "订单暂时不能退款");
        }
...
//        order.setRefundContent(wxPayRefundResult.getRefundId());
       ...
       if (userService.addUserIntegral( order.getUserId(),order.getActualPrice()) == 0){
           throw  new RuntimeException("更新用户积分失效");
       }
    ...
    public Object orderRemind() {
        ...
        return  ResponseUtil.ok(orderService.count());
