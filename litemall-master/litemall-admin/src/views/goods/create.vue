  ...
  <div class="app-container">
    ...
    <el-card class="box-card">
      ...
      <el-form ref="goods" :rules="rules" :model="goods" label-width="150px">
        ...
        <el-form-item :label="$t('goods_edit.form.is_dy')" prop="isNew">
          <el-radio-group v-model="goods.isDy">
            <el-radio :label="true">{{ $t('goods_edit.value.is_dy_true') }}</el-radio>
            <el-radio :label="false">{{ $t('goods_edit.value.is_dy_false') }}</el-radio>
          </el-radio-group>
        </el-form-item>
    ...
    <el-card v-if="goods.isDy" class="box-card">
      ...
      <el-table :data="segmentPrices">
        <el-table-column property="segment" :label="$t('goods_edit.table.segmentPrice_name')" />
        <el-table-column property="price" :label="$t('goods_edit.table.segmentPrice_value')" />
        <el-table-column property="integral" :label="$t('goods_edit.table.segmentIntegral_value')" />
        <el-table-column align="center" :label="$t('goods_edit.table.segmentPrice_actions')" width="100" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button type="danger" size="mini" @click="handleSegmentPriceDelete(scope.row)">{{ $t('app.button.delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>

      <el-dialog :visible.sync="segmentPriceVisiable" :title="$t('goods_edit.dialog.create_segmentPrice')">
        <el-form
          ref="attributeForm"
          :model="segmentPriceForm"
          status-icon
          label-position="left"
          label-width="100px"
          style="width: 400px; margin-left:50px;"
        >
          ...
          <el-form-item :label="$t('goods_edit.form.segmentIntegral_value')" prop="value">
            <el-input v-model="segmentPriceForm.integral" />
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="segmentPriceVisiable = false">{{ $t('app.button.cancel') }}</el-button>
          <el-button type="primary" @click="handleSegmentPriceAdd">{{ $t('app.button.confirm') }}</el-button>
        </div>
      </el-dialog>
    </el-card>
  ...
  methods: {
    ...
    handlePublish: function() {
      const finalGoods = {
        ...
        goodsDyPrices: this.segmentPrices
      }
    ...
    handleSegmentPriceShow() {
      this.segmentPriceForm = {}
    ...
    handleSegmentPriceDelete(row) {
      const index = this.segmentPrices.indexOf(row)
      this.segmentPrices.splice(index, 1)
      this.segmentNames.unshift(row.segment)
    },
