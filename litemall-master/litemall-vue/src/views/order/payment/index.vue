    ...
    <div class="pay_way_group">
      ...
      <van-radio-group v-model="payWay">
        <van-cell-group>
          ...
          <van-cell>
            <template slot="title">
              <img src="../../../assets/images/integral_pay.png" alt="积分支付" width="90" height="23"> 积分支付
            </template>
...
import {orderDetail, orderPrepay, orderH5pay, orderIntegrealpay} from '@/api/api';
...
export default {
  ...
  methods: {
    ...
    pay() {
      ...
      Dialog.alert({
        message: '你选择了' +  this.payWay === 'wx'? '微信支付':this.payWay === 'integral'?'积分支付':'支付宝'
      }).then(() => {
        if (this.payWay === 'wx') {
        ...
        else if(this.payWay === 'integral') {
          //todo : integralPay
          console.log("积分支付开始")
          orderIntegrealpay({ orderId: this.orderId })
              .then(res => {
                let data = res.data.data;
                console.log(res)
                window.location.replace(
