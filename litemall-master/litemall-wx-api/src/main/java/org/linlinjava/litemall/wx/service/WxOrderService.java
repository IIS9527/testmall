...
public class WxOrderService {
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        for (LitemallOrder o : orderList) {
            ...
            orderVo.put("duration", o.getDuration());
            orderVo.put("dateTimeBegin", o.getDatetimeBegin());
            orderVo.put("personAddress", o.getMessage());
...
//        String videoName = JacksonUtil.parseString(body, "videoName");
//        String datetimeBegin = JacksonUtil.parseString(body, "datetimeBegin");
        Integer grouponRulesId = JacksonUtil.parseInteger(body, "grouponRulesId");
        Integer grouponLinkId = JacksonUtil.parseInteger(body, "grouponLinkId");
//        String roomId = JacksonUtil.parseString(body, "roomId");




        if (cartId == null || cartId.equals(0) || addressId == null || couponId == null || message == null) {
            return ResponseUtil.badArgument();
        }
...
//        order.setDatetimeBegin(LocalDateTime.parse(datetimeBegin, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
//        order.setRoomId(roomId);
//        order.setVideoName(videoName);
        //!如果是dy商品
        if (!StringUtils.isEmpty(message) &&!cartId.equals(0)){
            LitemallCart cart = cartService.findById(cartId);
            order.setDuration(cart.getDuration());
            order.setDatetimeBegin(cart.getStartTime());
            order.setIntegral(cart.getIntegral().intValue());
            order.setDeviceNumber(Integer.parseInt(cart.getNumber().toString()));
        }
...
//        }
     ...
     */
    @Transactional
    public Object integralpay(Integer userId, String body, HttpServletRequest request) {
            ...
            if (!handleOption.isPay()) { return ResponseUtil.fail(ORDER_INVALID_OPERATION, "订单不能支付"); }

            LitemallUser user = userService.findById(userId);

            if (user == null|| user.getStatus() == 1 || user.getStatus() == 2){ return ResponseUtil.fail(ORDER_INVALID_OPERATION, "用户出错");}

            //判断余额
            if ( user.getIntegral().compareTo(order.getActualPrice()) <0 ){ return  ResponseUtil.fail(ORDER_INVALID_OPERATION, "余额不足"); }
            //减去实际价格
            if ( userService.reduceUserIntegral(user.getId(),order.getActualPrice()) == 0  ) { return WxPayNotifyResponse.fail("用户积分更新失效"); }

            //更新订单状态
            order.setOrderStatus(OrderUtil.STATUS_PAY);

            if (orderService.updateWithOptimisticLocker(order) == 0  ) { return WxPayNotifyResponse.fail("更新数据已失效"); }
