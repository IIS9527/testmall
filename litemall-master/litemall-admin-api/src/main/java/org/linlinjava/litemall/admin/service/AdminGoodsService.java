...
public class AdminGoodsService {
    ...
    private Object validate(GoodsAllinone goodsAllinone) {
        ...
        //是否抖音产品
        if (goods.getIsDy()){
            LitemallGoodsDyPrice[] goodsDyPrices = goodsAllinone.getGoodsDyPrices();
            if (goodsDyPrices.length<22) return ResponseUtil.badArgumentValue();

            for (int i = 0; i < goodsDyPrices.length; i++) {
                if (StringUtils.isEmpty(goodsDyPrices[i].getPrice()) || StringUtils.isEmpty(goodsDyPrices[i].getSegment()) ){
    ...
    public Object update(GoodsAllinone goodsAllinone) {
        ...
        // 特殊dy商品
        if (goods.getIsDy()){
            for (LitemallGoodsDyPrice goodsDyPrice : goodsDyPrices) {
                goodsDyPrice.setGoodsId(goods.getId());
                litemallGoodsDyService.updateDyProductPrice(goodsDyPrice);
            }
        }
    ...
    public Object delete(LitemallGoods goods) {
        ...
        litemallGoodsDyService.deleteByGid(gid);
        return ResponseUtil.ok();
    }
    ...
    public Object create(GoodsAllinone goodsAllinone) {
        ...
        if (goods.getIsDy()){
            for (LitemallGoodsDyPrice goodsDyPrice : goodsDyPrices) {
                goodsDyPrice.setGoodsId(goods.getId());
               litemallGoodsDyService.addDyProductPrice(goodsDyPrice);
            }
        }
    ...
    public Object detail(Integer id) {
        ...
        List<LitemallGoodsDyPrice> dyPrices = litemallGoodsDyService.queryByGid(id);
